...
import requests
import telebot
TOKEN = '1377435276:AAHE64maWJ9IWBg9I-bVduh651iJuoGVR7Y'
...
def send_msg_tsg(text):
	...
	bot.send_photo(chat_id=chat_id, photo=open('tersangka2.png', 'rb'))
...
def detect_and_predict_mask(frame, faceNet, maskNet):
	...
	orang = str(len(faces))
	link = "https://risetkanta.com/mask-detection/inputOrg.php?id=2&total="+orang
...
vs = VideoStream('http://192.168.137.123:8081').start()
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		if withoutMask > mask:
			img_item = "tersangka2.png"
			cv2.imwrite(img_item, roi_color)
			

			if (x == 1):
				print("mengirim")
				send_msg_tsg("The system has detect some offense at the second carriage")
				db =  requests.get('https://risetkanta.com/mask-detection/input.php?status=1&id=2')		
				x = 0
				y = 1
				counter2 = 11
				counter3= 5
